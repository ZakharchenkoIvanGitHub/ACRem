...
decode_results results;  // Somewhere to store the results
  ...
  StepSwing swing (18,19,12,13); //инициализация шторки
...
//myStepper.setSpeed(10); //Установка скорости шторки
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    if (description.length()) {
      ...
      model = NULL;
      pars(description); // отправляем строку в парсер
      update_display(); // обновляем дисплей
      }
...
//подготовка к парсенгу
void pars(String input_string){ 
  int len = input_string.length()+1;  //Преобразуем строку в массив символов 
  char str[len];
  input_string.toCharArray(str,len);
